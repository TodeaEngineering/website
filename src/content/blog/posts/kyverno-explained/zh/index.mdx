...
### 生成规则是怎么工作的,我又该如何让生成出来的资源保持同步?

生成规则会基于触发事件创建下游资源。在新的 `GeneratingPolicy` 上,把 `spec.evaluation.synchronize.enabled: true` 打开,Kyverno 就会保持下游资源同步——手动改动会被回退,源端变化会传播过去。把它保持在禁用状态(默认),Kyverno 就只在第一次生成,之后再不去碰。旧 `ClusterPolicy` 的等价开关是更简单的布尔值 `spec.rules[].generate.synchronize: true`。
